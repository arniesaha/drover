...
/// Builds a `NexusClient` from whatever configuration is available, so
...
public enum ClientFactory {
    ...
    public static func make(defaults: UserDefaults = .standard,
        ...
        if let override = ServerConfig.debugOverride() {
            return (NexusClient(config: override.config, token: override.token), override.config)
        }
        ...
        return (NexusClient(config: config, token: token), config)
    }
    ...
    public static func validate(config: ServerConfig, token: String,
        ...
        let client = NexusClient(config: config, token: token, session: session)
        do {
            ...
            return nil
        } catch let error as NexusError where error == .unauthorized {
