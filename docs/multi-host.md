...
If the config home is on an external volume that launchd cannot execute from,
put the staged runtime on the boot volume as well:

```toml
[update]
activation = "in_place"
in_place_venv = "~/.drover-venv"
runtime_root = "/path/on/boot/volume/drover-runtime"
```

Replace the example with a dedicated directory on the boot volume.
`runtime_root` must be an absolute path and is only supported with `in_place`
activation; symlink-mode service units still point to the default runtime
root. Service units generated by `install.sh` also use that default symlink:
before setting `runtime_root`, configure the harnessd unit and every sibling
in `restart_units` to execute binaries from `in_place_venv/bin`, then reload
and verify the installed units. The updater refuses activation if harnessd is
running from a different venv. First deploy a release that understands
`runtime_root` while using the old root, so the version available for rollback
understands it too. Rebuild staged venvs at the new root from their cached
artifacts; copying old venvs can leave entrypoints pointed at the old absolute
path. Then recreate `current` there and run `drover-server update --check` and
a staged version's `drover-server --version` as the launchd identity. Retain
the prior root until activation and rollback have been verified. Failed smoke
tests now log the executable, exit status, and a short stderr excerpt.
