...
Drover is a local-first cockpit, context store, and handoff layer for personal
and open-source agent harnesses. Sessions and traces from Claude Code/Codex-
style CLIs, OpenClaw, Hermes, Max, AgentWeave, and Mux can land in local
DuckDB + Parquet storage so another agent can replay, search, and hand off mid-task
without re-asking the user what's going on.

The default deployment is a single-user local install under `~/.drover/` on one
workstation. The dogfood deployment currently runs on the Mac Mini and accepts
shippers from a NAS and work MacBook, but there is no cloud component. The
GCP-era backend (BigQuery, Cloud SQL, Cloud Functions) was retired on
2026-05-09; its design is preserved in historical docs for reference.
...
local CLI harnesses without changing the durable context-store boundary. Drover
owns host/session metadata, auth, summaries, and handoff memory. Per-host
`drover-harnessd` daemons own the data plane: PTY/tmux processes, terminal
...
```
                              ...
                              ─────────────────────────────────────────────────────────
Sources           ship         ┌──────────────────────────────────────────────────────┐
─────────         ─────        │  ~/.drover/incoming/<host>/     ◄── rsync / launchd  │
NAS Claude  ──┐                │                                                       │
NAS OpenClaw  │                │  watcher  ─►  ingest  ─►  Parquet (Hive partitioned) │
Mac Mini      │  per-host      │                          ~/.drover/parquet/           │
...
├── drover.duckdb                      # tasks + summaries + briefs + embeddings + jobs
...
### `drover-collect` (per host)

A small Click app that scans a harness's local logs (Claude Code, Hermes,
OpenClaw, Pimono), parses them into `AgentEvent` rows, runs
`enrich_raw_repo_attribution()` against each event's `cwd` while git is still
resolvable locally, and rsyncs the resulting JSONL into the Mac Mini's
`~/.drover/incoming/<host>/`.

Runs every 5 minutes via launchd (macOS) or systemd (Linux). See [`install-shipper.md`](install-shipper.md).

### `drover-server` (local workstation, launchd/systemd/manual)

Long-lived process that hosts:

| Thread / endpoint | Job |
|---|---|
| `IncomingWatcher` | Picks up new JSONL files in `~/.drover/incoming/*`, calls `ingest_file()`, moves originals into `.processed/` |
...
| Tool | Backed by |
|---|---|
| `drover_handoff(repo_owner, repo_name, branch?)` | `tasks` + `session_summaries` + `active_sessions` — spans all branches when `branch` omitted |
| `drover_active_handoff(session_id)` | `active_session_briefs` — lazy-generates a fresh brief from the last 30 events if cache is stale (60s TTL); calls the LLM backend |
| `drover_fleet_status()` | `active_sessions` + `tasks` + latest user-message snippet |
| `drover_data_quality(hours?)` | `quality_snapshot()` — read-only status/score/categories/warnings self-check for agent handoff readiness |
| `drover_project_activity(project_key?, since?)` | `spans` grouped by repo, including AgentWeave-namespace spans where attribution can be derived |
| `drover_project_brief(repo_owner, repo_name)` | `project_briefs` |
| `drover_recent_sessions`, `drover_active_sessions`, `drover_session_summary`, `drover_session_replay`, `drover_search`, `drover_recall`, `drover_files_touched`, `drover_task_status`, `drover_session_close` | matching DuckDB views and tables |

## The attribution cascade

Most analytics in Drover collapse if events aren't pinned to a repo. The chain is:
...
6. **The brief worker pulls summaries by `(repo_owner, repo_name)`** through `tasks`, generates a project brief, writes to `project_briefs`. Enqueued via `drover-server brief` for any task with recent activity.
...
| AgentWeave spans | `drover-server otlp` (gRPC :4317), Tempo pull, or direct import | Provenance facts for spans, delegation, model/token/cost/cache, and routing evidence. Drover imports and links them for durable context recall without becoming a tracing dashboard; see [`agentweave-integration.md`](agentweave-integration.md). |

## DuckDB schema

Defined in [`src/drover/schema.py`](../src/drover/schema.py); bootstrap is idempotent (every `CREATE` is `IF NOT EXISTS` or `OR REPLACE`).
...
- **Idempotent boot.** Every `drover-server` start runs `bootstrap()` which creates tables and views from scratch. Drop the `.duckdb` file and the next start rebuilds everything from Parquet.
...
- `drover-server doctor` — row counts per table, per-(date, agent_id) partition counts, and processed-file drift between `~/.drover/incoming/<host>/.processed/` and the local store.
- `drover-server quality --json` — read-only data-quality snapshot with category scores for freshness, completeness, repo attribution, event identity, and derived-context/handoff readiness.
- `drover-server quality --prometheus` — the same snapshot rendered as Prometheus text metrics for Grafana scraping or node-exporter textfile export.
- `drover-server status` — config dump + table sizes.
- launchd / systemd logs — `~/Library/Logs/com.drover.server.{out,err}` on macOS.
...
`drover-server observatory` and the MCP tool `drover_pipeline_observatory` provide
...
HTTP server at `/` or `/ui`, with `/observability` and `/metrics` remaining
available for JSON and Prometheus consumers.

The goal is not a generic tracing dashboard. It is the operator view for Drover
itself: can the pipeline keep up, what did it produce, and can an agent trust
the derived context?

## Agent adoption

Drover is useful only when agents actually call it. The rollout contract is
tracked in [`agent-adoption.md`](agent-adoption.md):

- Every agent should have the Drover MCP endpoint configured.
- Every agent should know the smoke checks: `drover_data_quality`, `drover_handoff`, `drover_project_brief`, `drover_recent_sessions`, and `drover_session_replay`.
- Agents that operate Drover should also know `drover_pipeline_observatory` for saved artifact and project-readiness drilldown.
- Agent-local skills should point to the same Drover usage pattern instead of re-explaining it per harness.
...
The Meta Harness proposal makes Drover a mobile cockpit for a personal CLI
agent fleet while preserving Drover as the context plane:

- Drover server: host/session registry, auth, Redis-backed control events,
  transcript policy, summaries, handoffs, and observability.
- `drover-harnessd`: per-host data plane for PTY/tmux sessions on NAS, Mac Mini,
  and GPU PC.
...
- **Central skill registry and self-learning loop.** Drover can surface evidence, but it does not yet recommend skills or maintain a shared registry across agents.
